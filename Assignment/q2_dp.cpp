...
int main()
{
    ...
    //prompt for denominations, x, and max coins
    cout<<"Enter no. of denominations : ";
    int k,i,x,j;
    cin>>k;
    int deno[k];
    map<int,int> max_coins;
    cout<<"Enter the denominations"<<endl;
    for(i=0; i<k ; i++)
    {
        cout<<"Enter "<<i<<"th denomination:";
        cin>>deno[i];
        cout<<"Enter max coins for this denomination:";
        cin>>j;
        max_coins.insert(make_pair(deno[i],j));
    ...
    M.resize(k, vector<int>(x+1,0));
     ...
     //now , extract all the feasible solutions
...
while(!temp.empty())
{
    if(counter==0)
    {
        k= temp.back(); temp.pop_back(); counter++;
        while(1)
        {
